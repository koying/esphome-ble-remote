  ...
  namespace ble_client_hid
  {
    ...
    std::vector<HIDReportItemValue> HIDInputReportVariable::parse(uint8_t *report_data)
    {
      ...
      for (uint8_t i = 0; i < this->report_count; i++)
      {
        ...
        {
          ESP_LOGD(TAG, "Variable value out of range:  %d (s: %d / lM: %d / lm: %d): %s", value, this->report_size, this->logical_range.maximum, this->logical_range.minimum, esphome::format_hex(report_data, this->report_size));
          continue;
        }
    ...
    std::vector<HIDReportItemValue> HIDInputReportArray::parse(uint8_t *report_data)
    {
      ...
      for (uint8_t i = 0; i < this->report_count; i++)
      {
        ...
        if (value > this->logical_range.maximum || value < this->logical_range.minimum)
        {
          ESP_LOGD(TAG, "Array value out of range: %d (s: %d / lM: %d / lm: %d): %s", value, this->report_size, this->logical_range.maximum, this->logical_range.minimum, esphome::format_hex(report_data, this->report_size));
          value = 0;
        }
        if(value == 0){
